...
export interface VolumeStore extends VolumePathResolver {
   ...
   *
   * `title`, `body`, and `frontmatter` are always a full replace
   * (`frontmatter` defaults to `{}` when omitted — callers that want to
   * preserve existing keys must spread them in themselves, same as
   * `updateVolume`'s `frontmatter`).
   *
