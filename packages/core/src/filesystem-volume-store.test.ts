...
describe("FileSystemVolumeStore (filesystem-specific)", () => {
  ...
  test("on-disk layout matches <root>/volumes/<slug>/{volume.json,chapters/<slug>.md,index.json,evidence/}", async () => {
    const root = await makeTempRoot();
    try {
      ...
      expect(await Bun.file(join(volumeDir, "volume.json")).exists()).toBe(true);
